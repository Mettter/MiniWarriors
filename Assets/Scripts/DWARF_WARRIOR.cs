...
public class WarriorBoostReboot : MonoBehaviour
{
    ...
    private bool hasBoosted = false; // Ensures the boost happens only once per press

    private void Update()
    {
        // Check if P key is pressed and boost hasn't been applied
        if (Input.GetKeyDown(KeyCode.P) && !hasBoosted)
        {
            hasBoosted = true; // Mark the boost as applied
